...
__global__ void LocalStacks_global_kernel(Stacks stacks, CSRGraph graph, unsigned int * minimum, int* global_memory, Counters* counters, 
    unsigned int * pathCounter, int* NODES_PER_SM, int startingDepth) {
#else
__global__ void LocalStacks_shared_kernel(Stacks stacks, CSRGraph graph, unsigned int * minimum, Counters* counters, unsigned int * pathCounter, 
